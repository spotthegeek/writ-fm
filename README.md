...
- Speech is rendered by Kokoro TTS (fast), Google Gemini TTS (cloud), MiniMax TTS (cloud), or Chatterbox (voice cloning)
...
Voice samples are stored under `output/voice_samples/` and are generated once per voice. Kokoro, Google Gemini, and MiniMax all use the cached samples directly for playback.
